...
def test_keycloak_demo_fixture_declares_catalog_backed_iceberg_tables():
    ...
    assert {catalog["kind"] for catalog in fixture["catalogs"]} == {
        "delta_static",
        "iceberg_sql",
    }
    assert {table["backend"] for table in fixture["tables"]} == {"delta", "iceberg"}
    assert all(
        "table_path" not in table for table in fixture["tables"] if table["backend"] == "iceberg"
    )
    assert all("table_path" in table for table in fixture["tables"] if table["backend"] == "delta")
